...
import { brands, brandProducts } from "@/lib/data"
...
export default function BrandPage() {
  ...
  // Get brand info
  useEffect(() => {
    const brandInfo = brands.find((b) => b.link === `/brand/${slug}`)
    setBrand(brandInfo)

    // Get products for this brand
    const brandProductList = brandProducts[slug as keyof typeof brandProducts] || []
    setProducts(brandProductList)
    setFilteredProducts(brandProductList)
  }, [slug])
  ...
  }, [products, priceRange, selectedCategories, selectedRatings, sortBy])
                  ...
                  <div className="space-y-2">
                    {categories.map((category) => (
                      <div key={category} className="flex items-center gap-2">
                        ...
                        />
                        <Label htmlFor={`desktop-category-${category}`} className="capitalize">
                          {category}
                        </Label>
                      </div>
                    ))}
